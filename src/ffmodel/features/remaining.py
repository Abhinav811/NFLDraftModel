"""Rest-of-season overlay on the same board. Does not retrain.
...
from ..ingest.live_status import load_live_teams
from ..ingest.nflverse import load_pbp, load_schedules
...
def _current_teams(season: int) -> pd.DataFrame:
    teams = load_live_teams(season)
    if teams.empty:
        return pd.DataFrame(columns=["player_id", "live_team"])
    return teams.rename(columns={"team": "live_team"})
...
def load_remaining_schedule(season: int = PREDICT_SEASON) -> tuple[pd.DataFrame, int]:
    """Per-team remaining vs full schedule features. Empty frame if unavailable."""
    try:
        ...
        return full.merge(rem, on="team", how="left"), completed
    except Exception as exc:
        print(f"  Remaining SOS skipped ({exc})")
        return pd.DataFrame(), 0


def apply_ros_multipliers(df: pd.DataFrame, feat: pd.DataFrame) -> pd.DataFrame:
    """Scale frozen model_fp → ros_fp from leftover schedule. Same board, new points."""
    out = df.copy()
    model_fp = pd.to_numeric(out["model_fp"], errors="coerce")
    if feat is None or feat.empty:
        out["ros_fp"] = model_fp
        out["sos_factor"] = 1.0
        out["games_left"] = np.nan
        return out
    extra = [c for c in feat.columns if c != "team" and c in out.columns]
    out = out.drop(columns=extra, errors="ignore")
    out = out.merge(feat, on="team", how="left")
...
def attach_remaining_sos(df: pd.DataFrame, season: int = PREDICT_SEASON) -> pd.DataFrame:
    """Attach ros_fp and current team. Steal flags stay frozen. Preseason factor is ~1."""
    out = df.copy()
    ...
    feat, completed = load_remaining_schedule(season)
    out = apply_ros_multipliers(out, feat)
    model_fp = pd.to_numeric(out["model_fp"], errors="coerce")
